...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid fitColumns="false" checkbox="true" name="customerAdviceList" title="客户投诉" actionUrl="customerAdviceController.do?datagrid" idField="id" fit="true" queryMode="group">
  ...
  <t:dgCol title="进度"  field="astatus"  hidden="true" query="true" queryMode="single" dictionary="a_status" width="80"></t:dgCol>
   ...
   <t:dgToolBar title="修改" icon="icon-edit" url="customerAdviceController.do?goUpdate" funname="update" height="450"></t:dgToolBar>
   <t:dgToolBar title="删除"  icon="icon-remove" url="customerAdviceController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="customerAdviceController.do?goUpdate" funname="detail" height="450"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 <script src = "webpage/com/jinbo/customer/advice/customerAdviceList.js"></script>		
 <script type="text/javascript">
  $(document).ready(function(){
 		//给时间控件加上样式
 			$("#customerAdviceListtb").find("input[name='createDatetime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#customerAdviceListtb").find("input[name='createDatetime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#customerAdviceListtb").find("input[name='deDatetime']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
 </script>